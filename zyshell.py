...
class ShellCmd(cmd.Cmd, object): 
    ...
    def __getattr__(self, attr):
        ...
        the 'allowed' variable, and zyshell will react as if you had            \
    ...
    def check_secure(self, line, strict=None, ssh=None):
        ...
        Its purpose is to forbid the user to user to override the zyshell       \
        ...
        A warining counter has been added, to kick out of zyshell a user if he  \
    ...
    def mytimer(self, timeout):
        """ This function is kicks you out the the zyshell after      \
...
class CheckConfig:
    ...
    def getoptions(self, arguments, conf):
        """ This method checks the usage. zyshell.py must be called with a      \
        configuration file.
        If no configuration file is specified, it will set the configuration   \
        file path to /etc/zyshell.confelf.conf['allowed'].append('exit')
        """
        ...
        # ZYSHELLL_ARGS environment variable
        args = ['--config', conf['configfile']]
        if conf.has_key('logpath'): args += ['--log', conf['logpath']]
        os.environ['ZYSHELLL_ARGS'] = str(args)

        # if zyshell is invoked using shh autorized_keys file e.g.
        # command="/usr/bin/zyshell", ssh-dss ....
    ...
    def check_user_integrity(self):
        ...
        In case fields are missing, the user is notified and exited from zyshell.
        """
    ...
    def get_config_user(self):
        ...
        The zyshell command line is then launched!
        """
    ...
    def check_passwd(self):
        ...
        entered. 
        If the entered password is wrong, the user is exited from zyshell.
        """
